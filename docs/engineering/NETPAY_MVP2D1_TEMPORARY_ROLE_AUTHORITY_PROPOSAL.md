...
This proposal records the complete corrected design from the Package B physical discovery and second adversarial review, whose disposition was **READY FOR PROPOSAL**. B-01 through B-13 are **RESOLVED AT DESIGN LEVEL** only. Implementation, independent implementation review, evidence and conformance remain pending. The authorization to write this document is not authorization to register contracts, change architecture records, implement, migrate, assign a role or operate a pilot.
...
| `starts_at`, `review_due_at`, `expires_at` | Immutable finite UTC timestamptz values; start generated under lock, due at start plus 14 UTC days, expiry strictly after start and at most start plus 30 UTC days |
...
`starts_at` is generated after protected grant locks, not accepted as a caller's future or backdated value. `review_due_at = starts_at + 14 UTC days`; `starts_at < expires_at <= starts_at + 30 UTC days`. The authorized duration can be shorter. The interval is `starts_at <= t < expires_at`. Review is valid only when the last protected evaluation has `t < review_due_at` and `t < expires_at`. At the exact due boundary, absent a timely committed review, Intake is denied. At the exact expiry boundary, Intake is denied. There is no grace period or retry-induced deadline movement.

Commands take fresh samples after locks, before effects, and after flush. The sequence is: authoritative reread and validation; prepare transition/receipt/result/Event/audit in the same Session; flush; fresh snapshots and time; transition-specific final gate; hand commit to the owning UoW. The **last protected evaluation immediately before commit is the authorization/linearization point**. No further business mutation occurs after that gate. Review's staged `reviewed_at` cannot substitute for a timely final evaluation. Revoke's final gate validates the revocation transition, not a requirement that the now-revoked row still be active.
...
| --- | --- | --- |
| B1 | Explicit contract/profile allocation and PLANNED metadata | No executable handler or transport; independent contract evidence and conformance |
| B2 | Empty assignment/receipt schema, triggers/revisions, repository, PostgreSQL clock and central fail-closed resolver | No real assignment/backfill; all static recognition/assignment guards coherent; default composition cannot grant I |
| B3 | Internal Commands/receipts, Identity participant, session invalidation and reconcile | Single-UoW/concurrency/anti-bypass evidence; no productive transport or real assignment |
...
| --- | --- |
| B1 | Recheck all candidate IDs against normative documents and runtime projection; explicit names, scopes, inputs/results/error precedence/idempotency/Event relationships; operational PLANNED; QRY-001/QRY-003 and Identity profiles explicit; Package A's eight contracts unchanged; no handler or transport registration |
| B2 | Additive empty schema/head-parent verification; composite FK and uniqueness including terminal unrestored rows; immutable fields/history and empty assignments; four real PostgreSQL triggers, revision 1, real null-safe changes/no-ops, arbitrary-writer revision attempts, FK-driven changes, invalid OLD/overflow rejection; application-role privileges and trigger enablement; complete permission matrix and guards; clock boundary tests; all four productive calls/indirect consumers migrated coherently; no default I |
| B3 | Internal Assign/Review/Revoke/Reconcile atomic state/result/receipt/Event/audit; same-key replay/conflict and current admission; separate revocation/restoration; session snapshots and new authentication; Identity participant cannot own a transaction; all nine real PostgreSQL races; final gate semantics, commit failures and uncertain acknowledgment; no generic/HTTP/bootstrap/local/seed bypass or real assignment |
...
- One Dispatcher UoW and active Session per Command; no handler/participant commit, rollback, nested dispatch, savepoint, second Session or legacy committing authenticate call. Evidence and receipt share the same commit.
...
| B-05 | BLOCKER | Commit-free resolver and Session-bound Identity participant; legacy authenticate commits cannot split a B Command | RESOLVED AT DESIGN LEVEL | Single-UoW instrumentation and failure injection through handlers/participant |
| B-06 | BLOCKER | Fresh scalar snapshots and post-flush final evaluation; stale envelope or ORM map cannot authorize a later operation | RESOLVED AT DESIGN LEVEL | Stale identity-map/envelope, lock-wait and final-gate tests |
...
Only `docs/engineering/NETPAY_MVP2D1_TEMPORARY_ROLE_AUTHORITY_PROPOSAL.md` is created. No staging, commit, push, test execution, migration execution, configuration change, external account access or production operation is part of this delivery. `AUDIT_REPORT.md` remains unopened and unchanged.
