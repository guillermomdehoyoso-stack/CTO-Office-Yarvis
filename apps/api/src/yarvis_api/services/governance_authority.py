...
def evaluate_authority(
    envelope: IdentityAuthorityEnvelope, *, required_permission: str, target_organization_id: UUID
) -> None:
...
def _replay(
    db: Session, *, envelope: IdentityAuthorityEnvelope, key: str, fingerprint: str
) -> PrincipalMembership | None:
    command = db.scalar(
        select(PrincipalMembershipCommand).where(
            PrincipalMembershipCommand.organization_id == envelope.organization_id,
            PrincipalMembershipCommand.idempotency_key == key,
        )
    )
    if command is None:
        return None
    if command.request_fingerprint != fingerprint:
        raise ApplicationError(
            ApplicationErrorCode.CONFLICT, "idempotency key conflicts", {"reason": "idempotency_conflict"}
        )
    return db.scalar(
        select(PrincipalMembership).where(
            PrincipalMembership.id == command.membership_id,
            PrincipalMembership.organization_id == envelope.organization_id,
        )
    )
...
def activate_membership(
    ...
    *,
    ...
    if existing is not None:
        raise ApplicationError(
            ApplicationErrorCode.CONFLICT, "membership already exists", {"reason": "membership_terminal_or_existing"}
        )
    principal = db.scalar(select(Principal).where(Principal.id == principal_id, Principal.status == "active"))
    if principal is None:
        raise ApplicationError(
            ApplicationErrorCode.RESOURCE_NOT_FOUND, "not found", {"reason": "foreign_or_inactive_principal"}
        )
    membership = PrincipalMembership(
        principal_id=principal_id, organization_id=envelope.organization_id, role=role, status="active"
    )
    db.add(membership)
    db.flush()
    db.add(
    ...
    record_event(
        ...
        correlation_id=_correlation_uuid(envelope.correlation_id),
...
def revoke_membership(
    ...
    *,
    ...
    membership = db.scalar(
        select(PrincipalMembership).where(
            PrincipalMembership.id == membership_id, PrincipalMembership.organization_id == envelope.organization_id
        )
    )
    ...
    if membership.status == "revoked":
        raise ApplicationError(
            ApplicationErrorCode.CONFLICT, "membership is terminally revoked", {"reason": "membership_revoked"}
        )
    membership.status = "revoked"
    membership.revoked_at = datetime.now(timezone.utc)
    sessions = db.scalars(
        select(ProductiveSession).where(
            ProductiveSession.membership_id == membership.id,
    ...
    for session in sessions:
        ...
        session.revoke_reason = "authority_changed"
        record_event(
            db,
