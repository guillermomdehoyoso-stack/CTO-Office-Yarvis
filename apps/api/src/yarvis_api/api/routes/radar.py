...
def canonical_request(db: Session, request_id: UUID, organization_id: UUID) -> tuple[RadarRequest, RadarMerchant]:
    request = db.scalar(select(RadarRequest).where(RadarRequest.id == request_id, RadarRequest.organization_id == organization_id))
    if request is None:
        raise HTTPException(status_code=404, detail="not found")
    merchant = db.scalar(select(RadarMerchant).where(RadarMerchant.id == request.merchant_id, RadarMerchant.organization_id == organization_id))
    if merchant is None:
        raise HTTPException(status_code=404, detail="not found")
...
def dashboard(pending: bool | None = None, overdue: bool | None = None, product: str | None = Query(default=None, pattern="^(tpv|ecommerce)$"), merchant: str | None = None, priority: str | None = Query(default=None, pattern="^(low|normal|high)$"), db: Session = Depends(get_db), envelope: IdentityAuthorityEnvelope = Depends(authority_envelope)):
    envelope.require("radar.read")
    records = db.scalars(select(RadarMerchant).where(RadarMerchant.organization_id == envelope.organization_id).order_by(RadarMerchant.trade_name, RadarMerchant.id)).all()
...
def merchant_detail(merchant_id: UUID, db: Session = Depends(get_db), envelope: IdentityAuthorityEnvelope = Depends(authority_envelope)):
    envelope.require("radar.read")
    item = db.scalar(select(RadarMerchant).where(RadarMerchant.id == merchant_id, RadarMerchant.organization_id == envelope.organization_id))
    if item is None: raise HTTPException(status_code=404, detail="not found")
    read = merchant_read(db, item)
    requests = db.scalars(select(RadarRequest).where(RadarRequest.merchant_id == item.id, RadarRequest.organization_id == envelope.organization_id).order_by(RadarRequest.created_at.desc(), RadarRequest.id)).all()
    events = db.scalars(select(RadarActivity).where(RadarActivity.merchant_id == item.id, RadarActivity.organization_id == envelope.organization_id).order_by(RadarActivity.occurred_at.desc(), RadarActivity.id.desc())).all()
