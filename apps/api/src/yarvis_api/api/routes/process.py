...
def _principal(request: Request, db: Session, scope: str):
    ...
    envelope = AuthorityResolutionService().resolve(db, external_subject=authenticated.actor_id, selector=None, authentication_source=authenticated.authentication_method, correlation_id=authenticated.correlation_id or str(uuid4()))
    return process_principal_from_envelope(envelope, required_scope=scope)


@router.post("", response_model=ProcessDefinitionRead, status_code=status.HTTP_201_CREATED)
def create_process_definition(payload: ProcessDefinitionCreateRequest, request: Request, db: Session = Depends(get_db)) -> ProcessDefinitionRead:
    principal = _principal(request, db, "process.definition.manage")
...
def list_process_definitions(request: Request, db: Session = Depends(get_db)) -> list[ProcessDefinitionListItem]:
    principal = _principal(request, db, "process.definition.read")
...
def create_process_version(process_definition_id: UUID, request: Request, db: Session = Depends(get_db)) -> ProcessDefinitionRead:
    principal = _principal(request, db, "process.definition.manage")
...
def add_process_stage(process_definition_id: UUID, payload: ProcessStageCreateRequest, request: Request, db: Session = Depends(get_db)) -> ProcessDefinitionRead:
    principal = _principal(request, db, "process.definition.manage")
...
def update_process_stage(process_definition_id: UUID, stage_id: UUID, payload: ProcessStageUpdateRequest, request: Request, db: Session = Depends(get_db)) -> ProcessDefinitionRead:
    principal = _principal(request, db, "process.definition.manage")
...
def delete_process_stage(process_definition_id: UUID, stage_id: UUID, request: Request, db: Session = Depends(get_db)) -> ProcessDefinitionRead:
    principal = _principal(request, db, "process.definition.manage")
...
def add_process_transition(process_definition_id: UUID, payload: ProcessTransitionCreateRequest, request: Request, db: Session = Depends(get_db)) -> ProcessDefinitionRead:
    principal = _principal(request, db, "process.definition.manage")
...
def update_process_transition(process_definition_id: UUID, transition_id: UUID, payload: ProcessTransitionUpdateRequest, request: Request, db: Session = Depends(get_db)) -> ProcessDefinitionRead:
    principal = _principal(request, db, "process.definition.manage")
...
def delete_process_transition(process_definition_id: UUID, transition_id: UUID, request: Request, db: Session = Depends(get_db)) -> ProcessDefinitionRead:
    principal = _principal(request, db, "process.definition.manage")
...
def publish_process_definition(process_definition_id: UUID, request: Request, db: Session = Depends(get_db)) -> ProcessDefinitionRead:
    principal = _principal(request, db, "process.definition.manage")
...
def retire_process_definition(process_definition_id: UUID, request: Request, db: Session = Depends(get_db)) -> ProcessDefinitionRead:
    principal = _principal(request, db, "process.definition.manage")
