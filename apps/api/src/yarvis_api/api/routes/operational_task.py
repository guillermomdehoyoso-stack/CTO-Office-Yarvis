...
def create(x:TaskCreateRequest,request:Request):
 p=_p(request);return request.app.state.yarvis.operational_task_service.create(CreateOperationalTaskCommand(x.mission_work_item_id,x.title,x.description,x.priority,x.process_instance_id,x.process_stage_id,x.planned_start_at,x.due_at),_m(p,"create_operational_task",x.idempotency_key),p)
...
def transition(task_id:UUID,x:TaskTransitionRequest,request:Request):
 p=_p(request);return request.app.state.yarvis.operational_task_service.transition(TaskTransitionCommand(task_id,x.expected_version,x.status),_m(p,"transition_operational_task",x.idempotency_key,x.expected_version),p)
@router.post("/{task_id}/complete",response_model=TaskRead)
def complete(task_id:UUID,x:TaskCompletionRequest,request:Request):
 p=_p(request);return request.app.state.yarvis.operational_task_service.complete(CompleteOperationalTaskCommand(task_id,x.expected_version,x.completion_note),_m(p,"complete_operational_task",x.idempotency_key,x.expected_version),p)
@router.post("/{task_id}/cancel",response_model=TaskRead)
def cancel(task_id:UUID,x:TaskCancellationRequest,request:Request):
 p=_p(request);return request.app.state.yarvis.operational_task_service.cancel(CancelOperationalTaskCommand(task_id,x.expected_version,x.reason),_m(p,"cancel_operational_task",x.idempotency_key,x.expected_version),p)
