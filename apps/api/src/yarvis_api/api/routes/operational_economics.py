...
from yarvis_api.application.operational_economics_authority import economics_principal_from_envelope
...
def _principal(request: Request, db: Session, *, required_scope: str):
    ...
    return economics_principal_from_envelope(envelope, required_scope=required_scope)
...
def record_fact(payload: RecordEconomicFactRequest, request: Request, db: Session = Depends(get_db)) -> EconomicFactRead:
    principal = _principal(request, db, required_scope="economics.fact.record")
...
def correct_fact(fact_id: UUID, payload: CorrectEconomicFactRequest, request: Request, db: Session = Depends(get_db)) -> EconomicFactRead:
    principal = _principal(request, db, required_scope="economics.fact.correct")
...
def retrieve_history(subject_type: str, subject_id: UUID, request: Request, db: Session = Depends(get_db)) -> EconomicFactHistory:
    principal = _principal(request, db, required_scope="economics.read")
