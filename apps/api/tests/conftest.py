...
def clean_database(test_database):
    ...
    with SessionLocal() as session:
        session.connection().exec_driver_sql("TRUNCATE TABLE domain_events, requirement_fulfillments, intake_classifications, case_checklists, evidence, intake_items, checklist_requirements, checklist_templates, cases, people, organizations, document_types, case_types RESTART IDENTITY CASCADE")
        session.commit()
