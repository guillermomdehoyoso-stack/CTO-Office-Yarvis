...
const runtime = (manage = true) => ({ baseUrl: 'http://api.test', organizationLabel: 'Organización de prueba', capabilities: new Set(manage ? ['netpay.inbox.read', 'netpay.inbox.manage', 'netpay.master.read', 'netpay.master.manage'] : ['netpay.inbox.read']) });
...
describe('typed Netpay API client', () => {
  it.each([[403, 'forbidden'], [404, 'not_found'], [409, 'conflict']])('maps HTTP %i safely', async (status, kind) => {
    ...
    expect(fetchMock).toHaveBeenCalledWith('http://api.test/netpay/inbox/cases/case-1', expect.objectContaining({ credentials: 'include', headers: expect.not.objectContaining({ 'X-Yarvis-Subject': expect.anything(), 'X-Yarvis-Organization-Selector': expect.anything() }) }));
  });
});
